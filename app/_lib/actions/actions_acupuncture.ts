...
type AcupunctureUpdatePayload = {
  name: string
  paragraphs: string | string[]
  is_active: boolean
}

export async function updateAcupuncture(formData: FormData) {
  ...
  let payload: AcupunctureUpdatePayload
  try {
  ...
  // Odseky zjednotíme na pole stringov, aby frontend aj databáza používali rovnaký formát.
  ...
  if (updateError) {
    throw new Error(
      `Chyba pri aktualizácii Lekárskej akupunktúry: ${updateError.message}`,
    )
  }
  ...
  revalidatePath("/acupuncture")
  revalidatePath("/admin/acupuncture_settings")

  return {
    success: true,
    message: "Lekárska akupunktúra bola aktualizovaná.",
  }
}
