...
export default function JaluproSuperHydro_update_form({
  jaluproSuperHydroData,
  isAdmin,
}: JaluproSuperHydroUpdateFormProps) {
  ...
  function handleSubmit(formData: FormData) {
    startTransition(async () => {
      try {
        formData.set(
          "slug",
          (jaluproSuperHydroData as { slug?: string } | null)?.slug ??
            "jalupro-super-hydro",
        );
        formData.set(
          "data",
        ...
        console.error(error);
        toast.error("Chyba pri ukladaní Jalupro Super Hydro");
      }
    });
  }
  ...
  if (!jaluproSuperHydroData) {
    return (
      <div className="section-shell mx-auto w-full max-w-3xl p-6 text-center text-redDark">
        Dáta pre stránku Jalupro Super Hydro sa nepodarilo načítať.
      </div>
    );
  }
